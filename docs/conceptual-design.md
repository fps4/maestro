...
- **A requirement is an object, and the behavioural body is written in a constrained notation** *(new in v1.1, D45).* Traceability was asserted above and had nothing at the requirement end: the specification is versioned, and what it contains was opaque. **EARS** — five sentence patterns with a parseable grammar — is the notation, and each statement is addressable, so *"was my intent built?"* resolves to a named requirement, the golden case exercising it, and the artifact satisfying it. `platform-standards.md` §4.4 carries the adoption; PS3 §4.2 carries the model.
...
**The dial generalises, and v1.2 generalises it** *(D46).* What is stated here for oversight — *tighten but never loosen* — is the rule for **build standards** too: some are a **floor** the platform applies to everything and no client may decline; some are set by **consequence class**; some bind **conditionally** on facts about the client's own clients; and some a client may **elect** and pay for. `platform-standards.md` §4.5 carries the four classes and the gates on an election, one of which belongs to this document: **an elective the platform lacks the authority to enforce cannot be sold**, because §13.2 commits correctness only from N2 and §14.11 already names that trap. **The other half that belongs here is the price.** §6.1 there makes `inherited` the differentiator precisely because the application *cannot* get the control wrong — which requires the platform to do it for every application. **A control one client may decline is inherited by nobody**, so optionality is paid for out of what the platform can claim of the estate. Same asymmetry, arriving on the commercial side rather than the governance side, and it is the sentence a sales conversation will try hardest to skip.
...
| D45 | **The behavioural half of a specification is written in EARS, and a requirement is an addressable object versioned with the specification that contains it** | §4.2 and §19.1 have asserted requirement→artifact traceability since v0.3 with **nothing at the requirement end** — the artifact is versioned and its contents were opaque, so the edge ran to a blob. EARS earns adoption on **P2's** own terms: five patterns with a parseable grammar make *conformant* a check rather than an opinion, which is what separates a standard from guidance here. **The payoff is not readability** — an event-driven statement is an acceptance criterion in named slots, so direction 6's derivation and §14.4's *complete* golden-case suite both become countable. It **narrows A without closing it**: the prose-only option fails PS3 §4.1's scrubbability constraint, and a structured model with an EARS projection remains available. Bounded deliberately — EARS constrains *form*, so a pattern-conformant requirement can still be wrong, and a parse pass reported as a quality result is §14.11's laundering; the non-functional half stays with ISO 25010 on the facets | Proposed (v1.1) |
| D46 | **A build standard carries a class — floor, level by consequence class, conditional, or elected — and a client may tighten and may never loosen; an elected control is never inherited and never enters what the platform claims of the estate** | §12.5 established the asymmetry for oversight and it was never generalised, so every build standard has been universal **by assumption**. The commercial reading is the sharp one: §6.1 of `platform-standards.md` makes `inherited` the differentiator because the application *cannot* get the control wrong, which requires the platform to do it for **every** application — so a control one client may decline is inherited by nobody, and **optionality is paid for out of the estate-wide claim**. The second gate belongs to this document rather than that one: **an elective the platform lacks authority to enforce cannot be sold**, since §13.2 commits correctness only from N2 (D30) and §14.11 already names selling it earlier as the trap | Proposed (v1.2) |
...
1. **Specification format and management** — a platform service with API and MCP interfaces (D13, §4.2), all writes proposing (D14). *Still open: the specification's internal representation.* Controlled natural language, structured domain model, or hybrid. The property worth preserving from the remote reference is requirement→artifact traceability, which is what lets a non-technical owner confirm "was my intent built?" without reading code. **Narrowed in v1.1 and still open** (D45): PS3 §4.1's two constraints on **A** — mechanically scrubbable, and diffable to a non-technical owner — are jointly satisfiable by a controlled natural language and by neither of the alternatives alone, so the prose-only option is eliminated and **EARS is the CNL**. A structured model with an EARS projection remains available, which is why A is narrower rather than closed.
...
6. **Golden test derivation** — from acceptance criteria, with the coverage gap logged as a monitored risk (§15.4). ***Materially improved in v1.1 by D45.*** An event-driven EARS statement — *When `<trigger>`, the `<system>` shall `<response>`* — **is** an acceptance criterion with the trigger and the response already in named slots, so derivation stops being a translation someone performs and the coverage gap stops being a risk someone monitors: it becomes the ratio of requirements with a golden case to requirements without. §14.4's N4 flip requires *a golden-case suite that passes against the existing system first* and has never had a definition of a **complete** suite; this is the first one.
...
- **A.** Specification internal representation (from 1 above) — still the single largest undecided item, and it gates the rebuild. **Narrowed but not closed in v1.1** (D45): the notation for the behavioural half is settled, and what remains open is whether the *stored* form is the notation itself or a structured model the notation projects from. The residue is smaller and it is the half the composition plane cares about.
...
| 1.2 | 2026-08-08 | **D46 — §12.5's dial generalises from oversight to build standards, and the reason it matters is commercial rather than architectural.** *Tighten but never loosen* has been stated for oversight since v0.3.1 and never applied anywhere else, which left every standard in `platform-standards.md` §4 universal **by assumption** rather than by decision — the gap **V-E** had been holding open since that document's v0.1. Four classes now: **floor** the platform applies to everything and no client may decline, **level by consequence class**, **conditional** on facts about the client's own clients, and **elected**. **The half that belongs in this document is the price of the fourth.** §6.1 there makes `inherited` the differentiator precisely because a generated application *cannot* get the control wrong, and that requires the platform to do it for **every** application — so **a control one client may decline is inherited by nobody**, and optionality is paid for out of what the platform can claim of the estate. **The second gate is also ours**: an elective the platform lacks the authority to enforce cannot be sold, because §13.2 commits correctness only from **N2** (D30) and §14.11 already names selling it earlier as the trap. Five standards were adopted upstream in the same pass — C2PA for art 50 marking, decimal arithmetic for **P3**, RFC 9162 and 3161 for PS2's chain, SPIFFE for the workload principal that D45's neighbours T51 and V30 created and never gave an identity, and CSAF/VEX for §13.2's patch-latency commitment — and §12.5 gains the paragraph that connects them to the dial |
| 1.1 | 2026-08-08 | **D45 — the behavioural half of a specification gets a notation, and a requirement becomes an object.** §4.2 and §19.1 have asserted requirement→artifact traceability since v0.3 with **nothing at the requirement end**: the artifact is versioned and its contents were opaque, so the edge this design calls the property worth preserving ran from a version to a blob. **EARS** — five sentence patterns with a parseable grammar — is adopted on **P2's** own terms, since *conformant* becomes a check rather than an opinion, which is the line this document draws between a standard and guidance. **The argument that pays is not readability**: an event-driven statement is an acceptance criterion with trigger and response already in named slots, so **resolved direction 6** stops being a manual derivation with a monitored coverage gap and becomes a ratio, and **§14.4's N4 flip gets its first definition of a *complete* golden-case suite** — it has required one to pass against the existing system since v0.4 and never said what completeness meant. **Open question A is narrowed rather than closed**, which is the first movement on it since it was opened: PS3 §4.1's two constraints — mechanically scrubbable, and diffable to a non-technical owner — are jointly satisfiable by a controlled natural language and by neither alternative alone, so prose-only is eliminated while a structured model with an EARS projection remains available. **The adoption is bounded on purpose.** EARS constrains form, so a pattern-conformant requirement can still be unnecessary or wrong; a parse pass reported as a quality result is §14.11's laundering, and the non-functional half stays with ISO 25010 on the facets. Carried in `platform-standards.md` §4.4 (**V32**, **V33**), PS3 §4.2 (**S18**, **S19**), and the CI conformance job in `technical-design.md` §3.5 (**T54**) |
