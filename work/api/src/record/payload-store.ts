/**
 * The payload store — maestro-specs' ADR-0020, taken unchanged.
 *
 * The envelope carries tokens; what the service keeps in words — an item's title, a note, a
 * refusal's reason — is a payload: canonical JSON bytes
...
/** The key for a payload about a work item. */
export function itemPayloadKey(workspace: string, item: string, what: string): string {
  return payloadKey(workspace, 'work_item', item, what);
}
