...
# The workspace definitions ship in the image so `workspace:apply` can be run against a deployed
# container without a checkout. They are configuration, not content — the domain model itself.
COPY --chown=node:node config/workspaces ./config/workspaces
