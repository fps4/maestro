...
| Participant | A human attached to a product via a role (e.g. `architect`, `functional_reviewer`, `stakeholder`). Roles are per-product; a person can hold different roles in different products. |
...
| Gate | A point where the pipeline pauses for a human decision before continuing. maestro has two: functional (pre-code) and technical (design, and merge). |
