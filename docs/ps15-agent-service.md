...
**Companions:** `conceptual-design.md` (v0.8) is authoritative for *what* and *why*; `technical-design.md` (v0.7) for build order and substrate; `ps14-work-service.md` (v0.1) owns the commitments this service discharges; `ps7-data-service.md` (v0.2) holds everything this service produces that is not a governance fact; `ps1-identity-service.md` (v0.1) for the agent principal it runs as. Precedence runs in that order and this document is wrong where it conflicts.
**Realised as:** `agent-service`.
...
**A numbering conflict must be resolved before either answer is written down.** `ps7-data-service.md` §1.3 and H19 cite **T35** as the monorepo decision; `technical-design.md` v0.7 assigns **T35** to *PS14 added as the fourteenth service*. One of the two must be renumbered, and the monorepo decision is in any case not yet written in the technical design at all (PS7 §16.4 already records this). See §15.
...
**15.2 `technical-design.md` — the T35 collision, which blocks §1.2.** `ps7-data-service.md` §1.3 and H19 cite **T35** as the monorepo decision. The technical design v0.7 assigns **T35** to *PS14 added as the fourteenth service*. One must be renumbered, and the monorepo decision is not written in the technical design under any number — PS7 §16.4 already records this and it is now blocking a second document. It also still leaves `R-C` open and `ES-1` stale in `eip-refactoring-requirements.md`.
