...
- **Throughput is bounded by the human *decision*** — by design; maestro executes the merge on approval (ADR-0016), but nothing merges without the human's gate decision. The leverage is in spec/design/build. **Adopted scale through M3 (US-0024 H4):** the realistic operating envelope is **~1 product with ≤3 in-flight tasks** — there is no triage/queueing UX (batch-decide, snooze, gate-age telemetry) until the cross-product inbox (S6) lands in M3. A gate timeout escalates to the same architect (`config/reviewers.yaml` `on_timeout`), since no second human exists pre-M4; surfacing `gate_open_age` and an "open-gates" list is M3 inbox work. Don't plan dogfood load past this envelope until S6.
- **The differentiator is paid in vapourware until M4+ (US-0024 cross-cutting).** maestro's unique selling point is *product = many repos + many participants*, but v1 ships **single-repo, single-architect through at least M3**; multi-participant commercial onboarding (external functional reviewers) is M4. Calibrate dogfood expectations accordingly — the multi-party governance story (split review, per-product isolation) is exercised for real only when the first commercial product onboards.
...
- **The M3 docs agent depends on the M4 knowledge agent (US-0024 M4).** The docs agent (US-0016, M3) is specified as a *consumer* of the knowledge/context index (overview.md crew table), but the knowledge agent doesn't land until M4. When the M3 scoping doc opens it must resolve this: either land a **minimal knowledge index** in M3, or ship the docs agent on a documented heuristic (e.g. path-affinity from the diff) until M4. Decide upfront — don't discover it mid-M3.
