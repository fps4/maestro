...
# Versioned and encrypted, never public, never Object-Locked: the archive proves which bytes were
# accepted, and a payload lives here so that it can be erased (ADR-0020). The bucket itself is the
...
resource "aws_cloudwatch_metric_alarm" "api_5xx" {
  ...
  alarm_description   = "specs-service's API is returning server errors."
...
resource "aws_cloudwatch_metric_alarm" "relay_errors" {
  ...
  alarm_description   = "specs-service's relay errored; events stay pending in the outbox."
...
resource "aws_cloudwatch_metric_alarm" "relay_silent" {
  ...
  alarm_description   = "specs-service's relay has not run in fifteen minutes."
  namespace           = "AWS/Lambda"
...
resource "aws_cloudwatch_metric_alarm" "relay_refused" {
  ...
  alarm_description   = "The spine refused an event from specs-service; that workspace's relay is stopped until a person looks."
  namespace           = "maestro/spine"
  metric_name         = "Refused"
  dimensions          = { function = "relay", component = "specs" }
  statistic           = "Sum"
  period              = 300
  evaluation_periods  = 1
  threshold           = 1
