...
| **Artifact** | A lineage of a declared type, with a stable id | A cause analysis, a specification | A functional spec, a technical design |
| **Draft** | A **mutable** working copy. Where authoring happens. Not a record; nothing may cite it | An analysis an RCA run is writing | A spec being written by the crew |
| **Version** | An **immutable** snapshot of a draft. Superseded, never edited | A specification version | A revision put to a gate |
| **Type** | A declared artifact class: facet schema, body format, link types, attachment policy | `cause_analysis`, `change_record` | `charter`, `functional_spec` |
| **Facet** | A typed, schema-validated field. The only thing a gate or evaluator reads | Cause, blast radius, fix plan | `AC-N`, `NFR-N`, scope |
...
| **Body** | Authored content in a declared format. Rendered, diffed and searched — never evaluated | The narrative of an analysis | The markdown spec |
| **Attachment** | An image, PDF or file. Content-addressed in object storage, referenced from the body | A log excerpt, a graph | A diagram export |
| **Contributor** | A principal that edited a draft. Accumulates, and carries onto the version | The RCA run plus the person who confirmed its facets | A crew agent plus the architect |
| **Link** | A typed, directional edge between artifacts | Change record → specification | Spec → design |
| **Pinned link** | The one link per type resolved to a *version* at acceptance and frozen | The specification version a change record implements | The spec version a design implements |
| **Gate** | A declared decision point: what it decides on, who may decide, what must be true first | RCA review, intake, specification, release | Functional, technical design, technical merge |
| **Decision** | An immutable, attributed record of a gate outcome | The operations owner's decision at RCA review | Architect approval at merge |
...
| **Lifecycle** | Declared phases and transitions, each authorised by a gate | Under decision → Accepted → Closed | Draft → in review → approved |
| **Evaluation** | A verdict recorded against a version by an external evaluator. The service never computes one | The schema floor; a standards engine only in the regulated branch | Spec-adherence tests, the clarify pass |
| **Record sink** | The outbound port every state change is emitted to. Local by default; an external spine when configured | The spine — an S3 archive fed by a relay — which becomes authoritative | Local |
...
| **Register** | Everything in flight in a workspace, with lifecycle state | Everything under decision in a tenant | The backlog |
...
| Physical instance | *not modelled* | Deployment — a tenant is one, by default | Deployment = realm |
