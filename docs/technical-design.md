# maestro — Governed Application Platform — Technical Design

**Status:** Draft v1.5 — for refinement
**Repo:** maestro
**Companion:** `conceptual-design.md` (v1.2). That document is authoritative for *what* and *why*; this one is authoritative for *what we build first, on what, and in what order*. Where they conflict, the conceptual document wins and this one is wrong.
...
**Service designs:** one document per service as it is built — `ps1-identity-service.md` (v0.2), `ps2-record-spine.md` (v0.3), `ps3-specs-service.md` (v0.6), `ps7-data-service.md` (v0.5), `ps14-work-service.md` (v0.3), `ps15-agent-service.md` (v0.2), `ps16-runtime-service.md` (v0.1). **Standards content:** `platform-standards.md` (v0.8) consolidates Tier 1, the build standards, and the frameworks the platform can claim; `uc1-nl-construction.md` (v0.3) models the first domain. Both are subordinate to this document and to the conceptual design. **Diagrams:** `c4-diagrams.md` (v0.4), derived from these documents and never authoritative over them. *(**Eight of the ten versions in this paragraph were stale** as of v1.3 — which is the same defect §3.4's import rule 6 exists to catch one field over, and it is not yet checkable because a version is a claim about a file's contents rather than about its existence.)* This document stays authoritative for the service *set*, the order, and the substrate; each service design is authoritative for its own internals.
...
|---|---|
| **The platform consumes rather than builds it** — someone else owns the roadmap and the ADRs | `identity-service` (PS1 auth), in `../identity-service`; `specs-service` (PS3 + PS4), in `../maestro-specs` |
...
**None of that tree exists yet, and the estate currently contradicts it in two places** *(three until 2026-09-15)*. Stated because a topology written against nothing is the easiest kind of decision to write and the easiest to have already been overtaken:

| On disk today | Against T53 |
|---|---|
| `maestro/` holds `docs/` and nothing else | Every directory above is prospective, and **so are the five import rules** — a rule with no tree to bind is `platform-standards.md` **V4**'s failure mode arriving *before* the code rather than after it, which is the only version of it that is still cheap to fix |
| ~~`mstr-data/` and `mstr-work/`~~ — **removed 2026-09-15** | Were the reversed plan, sitting where someone would `git init` it. Both service designs carried a `Repo:` line naming them; both were corrected (PS7 v0.4, PS14 v0.2), and the directories — **the part a document cannot fix** — are gone with the estate rename. Kept as a row because the mismatch was recorded rather than quietly fixed, and the fix should be too |
| `maestro-specs/` — a real repository, first build merged | Correct and outside, per row 1 of the table above. **The name is `maestro-specs` and the service is `specs-service`**, and PS3 used three names for the pair across two adjacent header lines until v0.4 |
...
### 3.5 The CI conformance job

*New in v1.3 (**T54**), and overdue. `platform-standards.md` has requested this job four times — **13.7** for §4's build standards and **V4**'s six invariants, **13.17** for `PLAT-AI-*`, **13.21** for `PLAT-RUN-*`, and **13.23** for the EARS parse — and §3.4 above requires it for import rule 6. **Four requests and a sixth import rule are a job with an owner, not a fifth mention.***

**The defect this closes is V4's and it is worth naming precisely.** *"Enforced by CI"* appears against Tier 1 invariants in a document that names no pipeline, and an invariant enforced by a sentence is enforced by whoever last read it. This section does not write the checks — it establishes the job, its groups, and the rule that keeps it honest.

| Group | What it checks | Binds |
|---|---|---|
| **Repository** | §3.4's six import rules | **Rule 6 today; rules 1–5 when the tree exists** |
| **Build standards** | §4.2 of the standards document — ASVS level, SBOM present and signed, SLSA level, licence policy, WCAG target — at the consequence class §4.1 resolves | With the composition plane |
| **Tier 1 invariants** | The `PLAT-*` catalogue, including `PLAT-AI-*` and `PLAT-RUN-*` | Per invariant, as each gets a named check |
| **Specification** | The **EARS parse** over a proposed specification body: statements parsed, statements unparsed, per version | With PS3 §4.2's requirement index |

**The specification group is different in kind from the other three and the difference matters.** The first three read an *artifact the platform emits*; this one reads an *input the platform was given*. Its output is therefore **two numbers and never a verdict** — parsed and unparsed — because `platform-standards.md` §4.1 permits unparsed lines at low consequence and requires them counted, and because reporting a parse pass as a quality result is the laundering §4.4 there spends a paragraph forbidding. **The job publishes counts; the gate decides what they mean.**

**The rule that keeps the job honest, and it is the one thing here that is not administrative.** An invariant the **substrate** makes unrepresentable gets **no check**, and is recorded in the job's manifest as *deliberately unchecked, with the enforcing mechanism named*. Four of §3.8's eight `PLAT-RUN-*` invariants are in this class — no general egress, nowhere to keep state, invocation-scoped credentials, a handle bound per invocation — and they are the strongest assurance position the platform holds (V31). Without the manifest entry, the next reader adds a redundant lint and the reader after that removes the network policy because *"CI covers it."*

**What this section does not do.** It does not enumerate a check per invariant; **V4 stays open until each `enforced_by: ci` entry names one or withdraws the claim**, and forty-six invariants is not a list to invent here. What changes is that the claim now has somewhere to be true.

**The check set is resolved per tenant, not fixed** *(new in v1.4, **T55**, from `platform-standards.md` **V39**).* That document's §4.5 gives every build standard a class — **floor**, **level by consequence class**, **conditional**, **elected** — so the set this job runs against a given specification is the union of four resolutions and not a constant. Three consequences, and the third is the one that will bite:

- **Floor and conditional resolve from the specification and the tenant binding.** No new machinery: the predicate is `applies_when`, which the standard object has always carried.
- **Level and elected resolve from consequence class and the binding's `elected_standards`.** **W8's rule applies unchanged — one resolver, and it is not two.** A second resolution path here would put the CI job and PS5 on different answers for the same application, which is the failure mode a conformance product can least afford.
- **A failing check must name which class put it in scope.** *ASVS L3 failed* is unactionable when nobody can see whether L3 arrived from the consequence class or from something the tenant bought. It is also the input to **V40's** distinction between what the platform claims of everything and what one tenant elected.

### 3.6 Three floor standards with a substrate face

*New in v1.4 (**T56**). `platform-standards.md` **V35**, **V36** and **V37** adopt standards that are not the composition plane's to satisfy — they are properties of what we build, and a standard nobody implements is **V4**'s defect arriving through a different door.*

| Standard | Binds | Consequence here |
|---|---|---|
| **IEEE 754 decimal, declared rounding** (V35) | **AE3**, and any engine producing a regulated number | The calculation engine's numeric type is a **build constraint, not a library preference**. Rounding is pack input because Dutch payroll and tax instruments specify it, so AE3 reads it rather than choosing it |
| **RFC 9162 Merkle log, RFC 3161 timestamps** (V36) | **PS2**'s sealer and R12's `export` | The sealer emits a standard tree; the exported verifier (**T24**) is then a checker of a published format rather than of ours. **This is the change that makes P9's independence claim true rather than asserted** |
| **SPIFFE / SPIRE** (V37) | **PS1**, PS16, PS10 | **T44's invocation-scoped credential is a SPIRE issuance.** T51's workload principal gets an identity document instead of a scheme, and it survives the substrate change T22 exists to keep possible |

**One ordering consequence.** V36 touches PS2's sealer, and PS2 is first-wave — so this is cheap now and expensive later, in the same way T10's SBOM was. A chain resealed under a new format is a migration of the one artifact whose whole value is that it was never rewritten.
...
| T54 | **The CI conformance job exists as one job with four groups — repository, build standards, Tier 1 invariants, specification — and an invariant the substrate makes unrepresentable gets no check and a manifest entry naming what enforces it instead** | `platform-standards.md` asked for this job four times (13.7, 13.17, 13.21, 13.23) and §3.4's import rule 6 binds today with nothing to run it. **V4's defect is that *enforced by CI* names no pipeline**, which makes the enforcement whoever last read the sentence. The fourth group is different in kind — it reads a specification the platform was *given* rather than an artifact it *emitted*, so it emits **counts and never a verdict** (V32). The manifest rule is the load-bearing half: four `PLAT-RUN-*` invariants are enforced by the substrate and are the platform's strongest assurance position (V31), and an unrecorded absence of a check is how someone later deletes the network policy because CI appeared to cover it |
| T55 | **The CI conformance job's check set is resolved per tenant — floor, level by consequence class, conditional, and elected — through one resolver, and a failing check names which class put it in scope** | `platform-standards.md` **V39** gives every build standard a class, so §3.5's set is the union of four resolutions rather than a constant. **W8's rule is the load-bearing half**: a second resolution path would put this job and PS5 on different answers for the same application, which is the one disagreement a conformance product cannot have. Naming the class on a failure is not ergonomics — it is the input to **V40's** separation of what the platform claims of every application from what one tenant bought |
| T56 | **AE3's arithmetic, PS2's chain format, and workload identity are substrate decisions and are named: IEEE 754 decimal with pack-supplied rounding, RFC 9162 with RFC 3161, and SPIFFE** | Three of `platform-standards.md`'s floor additions (**V35**, **V36**, **V37**) are properties of what *we* build rather than of what the composition plane emits, and **a standard nobody implements is V4's defect through a different door**. V36 is the one with a clock on it: PS2 is first wave, and a chain resealed under a new format later is a migration of the single artifact whose entire value is never having been rewritten |
...
| 1.3 | 2026-08-08 | **§3.5 added — the CI conformance job, and it is overdue rather than new.** `platform-standards.md` has requested this job four separate times — **13.7** for §4's build standards and **V4**'s six invariants, **13.17** for `PLAT-AI-*`, **13.21** for `PLAT-RUN-*`, **13.23** for the EARS parse — and §3.4's import **rule 6 binds today** with no pipeline to run it. Four requests and a binding rule are a job with an owner, not a fifth mention, and **V4's actual defect is that *enforced by CI* names no pipeline**, which leaves the enforcement to whoever last read the sentence. **T54** establishes the job with four groups and does *not* enumerate a check per invariant; **V4 stays open** until each `enforced_by: ci` entry names one or withdraws the claim, and what changes is that the claim now has somewhere to be true. **The fourth group is different in kind and the difference is the point**: repository, build-standards and Tier 1 checks read an artifact the platform *emits*, while the specification group reads an input the platform was *given*, so it publishes **two numbers — parsed and unparsed — and never a verdict**. That follows the standards document's §4.1, which permits unparsed lines at low consequence and requires them counted, and its §4.4, which forbids a parse pass being reported as a quality result. **The manifest rule is the load-bearing half**: an invariant the substrate makes unrepresentable gets no check and an entry naming what enforces it instead — four `PLAT-RUN-*` invariants are in that class and are the strongest assurance position the platform holds (**V31**), and an unrecorded absence is how a network policy gets deleted later because CI appeared to cover it. **The service-design version list in the header was corrected and eight of its ten versions were stale**, which is import rule 6's defect one field over and is *not* checkable the same way — a path either resolves or does not, while a version is a claim about a file's contents. **Change-log rows reordered**: five of thirteen were out of sequence (0.8 before 0.7, and 1.2, 1.1, 1.0, 0.9 in that order at the tail), the same defect `platform-standards.md` v0.4 fixed in itself and v0.7 fixed again |
| 1.4 | 2026-08-08 | **§3.5's job stops being a constant and §3.6 gives three adopted standards an implementer.** `platform-standards.md` **V39** gives every build standard a class — floor, level by consequence class, conditional, elected — so **T55** makes the check set the union of four resolutions, through **one resolver**: a second path would put this job and PS5 on different answers for the same application, which is the single disagreement a conformance product cannot survive. A failing check now names **which class put it in scope**, because *ASVS L3 failed* is unactionable when nobody can see whether L3 came from the consequence class or from something the tenant bought — and that attribution is the input to **V40's** separation of the platform's baseline claim from one tenant's purchase. **T56 is the other half and it closes a gap that opens whenever a standard is adopted upstream**: three of the five floor additions are properties of what *we* build rather than of what the composition plane emits, and **a standard nobody implements is V4's defect arriving through a different door**. AE3's arithmetic becomes IEEE 754 decimal with rounding supplied by the pack rather than chosen by the engine, since Dutch payroll and tax instruments specify it; PS2's sealer emits an **RFC 9162** tree with **RFC 3161** timestamps, which is what makes T24's exported verifier a checker of a published format instead of of ours; and workload identity becomes **SPIFFE**, whose issuance model T44's invocation-scoped credential already is. **V36 has a clock on it**: PS2 is first wave, so this is cheap now in exactly the way T10's SBOM was, and a chain resealed under a new format later is a migration of the one artifact whose whole value is never having been rewritten |
| 1.5 | 2026-09-15 | **Estate renamed `mstr-*` → `maestro-*`.** The GitHub repositories `fps4/mstr` and `fps4/mstr-specs` are now `fps4/maestro` and `fps4/maestro-specs`, and the two local repositories follow; the two empty directories — `mstr-data/`, `mstr-work/`, T53's reversed plan — are **removed rather than renamed**, which closes the estate row that called them the part a document cannot fix. §3.4's placement table and estate table updated, and the title drops the codename. Recorded while there: `maestro-specs` is no longer *one commit* — its first build (steps 1–9, the catalogue, MCP) is merged, which §3.0.1's *designed-but-unbuilt* and PS3 §1.2 do not yet reflect, and is left for a content revision rather than fixed in a rename. The ds1 runtime identifiers inside `maestro-specs` — compose project, container names, `AUTH_AUDIENCE`, the identity client IDs, the bucket — still read `mstr-specs`; that is a deploy change with an `identity-service` seed on the other side, deliberately not made here. PS3 v0.6, PS7 v0.5, PS14 v0.3. No service, order, placement, or rule changed |
