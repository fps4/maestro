...
**Repo:** `maestro/services/work-service`. *(Corrected in v0.2 — this line read `mstr-work` and a `Repositories/fps4/mstr-work` directory — since removed — was created against it. **T53 reverses that plan**: a repository is earned by a consumer, and this service has one. The empty directory is the reversed plan still sitting on disk.)*
...
| 0.3 | 2026-09-15 | The empty `mstr-work` directory recorded in the header and in v0.2 is removed with the estate rename (technical design v1.5, §3.4). The reversed plan no longer sits on disk. Nothing else changed |
