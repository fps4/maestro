...
| **Intent intake mechanism.** With Telegram/Slack off the M1 critical path, how does the architect submit intent — a workspace "new task" affordance, or a CLI/`maestro` command seeding the event log? | @architect | Open — **deferred to US-0010 acceptance.** A minimal workspace affordance or a `maestro` CLI seed both satisfy US-0010's acceptance criteria; the intake mechanism is the open call at story acceptance. |
