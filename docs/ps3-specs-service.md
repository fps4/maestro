...
**Realised as:** the service **`specs-service`**, in the repository **`../maestro-specs`** — adopted, and one of the two repositories T53 places *outside* the maestro monorepo, because the platform consumes rather than builds it. That repository owns its own internals and its ADRs; this document is authoritative for what maestro requires of it.
**Naming, fixed in v0.4:** the service and its repository have different names and this document used three. The `Repo:` line said `mstr-specs`, the `Realised as:` line two rows below said `../specs-service`, and only the first resolves on disk. **`specs-service` is the service; `maestro-specs` is the repository** *(renamed from `mstr-specs` in v0.6)*. Body references say `specs-service` where they mean the service and `../maestro-specs` where they mean the checkout — never the other way round, and never a path that does not exist.
...
| 0.6 | 2026-09-15 | Repository renamed **`mstr-specs` → `maestro-specs`** (technical design v1.5, estate rename); the `Realised as:` line, the naming rule, and §1.2's path updated. The service is still `specs-service`. §1.2 still calls it *designed-but-unbuilt* and that is now false — the first build is merged — but the gap assessment that depends on it is a content revision, not a rename. No gap, decision, or open item changed |
