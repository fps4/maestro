...
- **Plain-language summary: agent-authored vs on-the-fly toggle?** Either the spec/design agent writes a "for non-technical readers" paragraph at the top of each artefact (one writer, costs tokens once), or the workspace lets the reviewer click "show me the plain version" and renders it on demand (re-runs LLM each click, can fall behind the artefact). Affects P1 and the agent contract — needs an ADR.
- **Feedback-bundle contract.** When *request-changes* fires, what exactly does the agent receive? An ordered list of `(anchor, comment, suggested_change?)`? Free-text plus anchors? Both? Affects the orchestrator ↔ crew contract; settle before US-0010/US-0013 finalise.
