...
**Name:** `data-service`, per **T32**. One deployable in the **maestro monorepo**, under `products/data-service` (§1.3 — the decision it cites is not yet written upstream, see §16.4) — not its own repository. **Started new, not evolved out of `event-integration-platform`** (**T33**), which is now retired as a candidate entirely.
