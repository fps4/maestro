/**
 * Item access bound to a key prefix — the mechanism under every handle (ADR-0006, ADR-0021).
 *
 ...
 * says whether re-running the whole read–decide–write is the right response (the outbox counter
 * moved: someone else recorded first) or the caller must be told (the version is no longer
 * proposed; the draft's revision is stale).
 */
...
export class Items {
  ...
  /** Put an item that must not exist yet: the key is the unique value (ADR-0021 §3). */
 ...
 * (ADR-0021 §2). Reads happen before; every write carries the condition that makes the read
