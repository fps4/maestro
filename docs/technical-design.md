...
**Status:** Draft v0.9 — for refinement
**Repo:** mstr
...
**Service designs:** one document per service as it is built — `ps1-identity-service.md` (v0.1), `ps2-record-spine.md` (v0.2), `ps3-specs-service.md` (v0.2), `ps7-data-service.md` (v0.2), `ps14-work-service.md` (v0.1), `ps15-agent-service.md` (v0.1). **Standards content:** `platform-standards.md` (v0.3) consolidates Tier 1 and the frameworks the platform can claim; `uc1-nl-construction.md` (v0.2) models the first domain. Both are subordinate to this document and to the conceptual design. **Diagrams:** `c4-diagrams.md` (v0.2), derived from these documents and never authoritative over them. **Refactoring requirements:** `eip-refactoring-requirements.md` (v0.1), subordinate to this document and to the PS7 design. This document stays authoritative for the service *set*, the order, and the substrate; each service design is authoritative for its own internals.
...
**Read the graph in §5.1, not the numbers.** PS1–PS10 were numbered in a dependency-respecting order and that order is now broken: PS5 depends on PS11, which is numbered after it. PS11–PS13 are appended in the order they were *derived* rather than sorted into the sequence, because the PS numbers are referenced throughout §9's decision log and renumbering would invalidate those references. The numbering is not a priority order either — PS9 and PS10 depend only on PS1 and are deferred by trigger (§6), not by dependency.
...
Four consequences, all of them recorded rather than resolved here:
...
| `event-integration-platform` | **Splits** — `data-service` (**PS7**) and `exchange-service` (**AE6**) | **T33.** v0.4 assigned it to PS7 *(event shape)* and AE6 at once, which **T19 forbids**: connectors cross a trust boundary and enforce capability grants, and that is governance, not data. So the reuse answer is neither adopt nor replace — the repository splits. Note also that its documentation describes a target its code does not implement (its own `LIMITATION-0001`): the Flink runtime and the AI assist layer are backlog, and what exists is a control plane over Kafka, an ingress/egress surface, and DLQ/replay. Requirements, inventory, and sequencing are in `eip-refactoring-requirements.md`. Still explicitly **not a PS2 candidate**: T5 forbids personal data in the record spine and an integration platform carries business events that routinely contain it |
...
- **Deployment-per-tenant.** On Compose, another tenant is another project and a port. On Kubernetes plus AWS it is a namespace, an RDS instance, and MSK capacity — a cost floor that §3.1's twelve-person aannemer cannot carry. `identity-service`'s ADR-0018 already chose this model for a different problem, so it is both the easiest PoC path and the nearest precedent. **§3.3 closes this**, and the PoC must run the model §3.3 settles rather than the one Compose makes easy.
...
**PS9 and PS10 have gates even though they are deferred**, because they were the only two services with no definition of done — which is how a deferred thing arrives under pressure with its criteria invented on the spot.
...
- **T-O.** Whether PS13's ledger is a projection of PS2 or its own store. Deployment events are governance events and belong on the spine; SBOMs are bulky, immutable, and referenced rather than queried, which is the T5 shape argument applied to a different payload.

*Opened in v0.6:*

- **T-P.** **Where transformation lives** — the largest open item in the `event-integration-platform` split, and the one that decides how big each of the two services is. Flink SQL at an ingress boundary is AE6; Flink SQL over a PS7 stream is arguably PS7's streaming access mode. That repository makes transformation the centre of the product and does not distinguish the two cases. Must close before phase 3 of the refactor, and it decides whether that repository's ADR-0002 moves or is superseded. *Also carried as **H-A** in the PS7 design and **R-A** in the refactoring requirements.*
- **T-Q.** Whether `exchange-service` gets its own design document before it is built, or whether the boundary requirements plus an ADR set are sufficient. AE6 is deferred in §5.2's engine order, which argues for the lighter answer — but AE6 is also the enforcement point for P8's capability grants alongside PS9, which is the argument the other way.
