 ...
 * the projection.
 *
 * The verifier runs first, the target must be empty or `--force` drops it, and memberships are
 * not rebuilt: re-apply them from the tenant's configuration afterwards (ADR-0020 §5).
 */
...
async function main(): Promise<void> {
  ...
  // The archive is read, never relayed to: this process replays.
  ...
  try {
    ...
    console.log(
      `Rebuilt workspace \`${report.workspace}\` from ${report.events} events: ` +
        `${report.artifacts} artifacts, ${report.versions} versions, ${report.decisions} decisions, ` +
        `${report.evaluations} evaluations, ${report.questions} questions, ${report.drafts_reopened} drafts reopened; ` +
        `${report.acceptances.checked} acceptances refreshed.`,
    );
