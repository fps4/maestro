...
**Realised as:** a **composite across two repositories**, which is why this document alone has no single `Repo:` line. The authentication half is the adopted service **`identity-service`**, in `../identity-service` — one of the two T53 places *outside* the maestro monorepo, because the platform consumes rather than builds it. The **governance identity layer is maestro's own**, in `maestro/services/identity-layer`, and is inside like everything else. *(Corrected in v0.2 — this line read `mstr-idp = 'identity-service'`, which names a repository that does not exist, an equality that is not one, and a split this document's own §-structure has described since v0.1.)*
**Companions:** `conceptual-design.md` (v1.0) is authoritative for *what* and *why*; `technical-design.md` (v1.2) for build order, substrate, and the tenancy model in its §3.3; `ps2-record-spine.md` (v0.3) records against the principal model defined here. Precedence runs in that order and this document is wrong where it conflicts.
...
| Version | Date | Change |
|---|---|---|
| 0.2 | 2026-08-06 | **The `Repo:` line replaced with `Realised as:`, because PS1 is the one service that cannot have a single repository and the old line was trying to say so in punctuation.** It read `mstr-idp = 'identity-service'` — a repository name that does not exist, joined by an equals sign to the one that does. T31 has split this service into an adopted authentication half and a maestro-built governance half since v0.5, and **T53 puts the two on opposite sides of a repository boundary**: `identity-service` is outside because the platform consumes rather than builds it, and the identity layer is inside like everything else. Found by the sixth CI rule added in technical design v1.2 — *every `Repo:` line resolves* — applied by hand before the pipeline that will apply it exists. Companion versions corrected. No gap, assessment, decision, or open item changed |
