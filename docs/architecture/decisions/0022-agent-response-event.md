...
- **Irreconcilable bundle items — a fourth action (US-0024 M1).** The `addressed | deferred | rejected` trichotomy has no "these anchored comments conflict and I can't satisfy both." Multi-reviewer (US-0017+) makes conflicting anchors likely; today the agent's only escape is `rejected` → a new bundle, which loses the conflict context. **Decision (deferred to M3, when multi-reviewer lands):** add a fourth action `escalate_to_reviewer` carrying a required `conflict_note` (and the conflicting comment_ids), surfaced at the gate so a human reconciles. Cheaper interim option, also deferred: the workspace warns at bundle-creation when two anchored comments on the same locator materially conflict. Not in M1/M2 (single architect — no conflicting reviewers yet); recorded here so the trichotomy isn't mistaken for complete.
