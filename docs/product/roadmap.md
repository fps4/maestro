...
M0–M2 are the MVP. M3–M4 build on it. The webapp is delivered in the concept's steps **S1 Read → S2 Discuss → S3 Decide → S4 Artefacts → S6 Inbox**, woven across milestones. Story-level scope is on the [feature board](feature-board.md).

| Milestone | Goal | Stories | Exit criteria |
|---|---|---|---|
| **M0 — Foundation + surface backbone** | An audited model egress, a control plane that boots, and the workspace reachable | US-0021, US-0020 (core), US-0001, US-0030 (**S1**) | maestro boots, connects GitHub + Claude + the **webapp** (`component-auth`, over the Cloudflare Tunnel); the GitHub adapter **refuses to merge without a valid approval event** (ADR-0016); the workspace **renders a repo-sourced spec read-only** (S1); an LLM call is recorded |
...
| Human surface | The **maestro workspace** — `component-auth` + the Cloudflare Tunnel/Access (ADR-0012/0015), an MIT/open base (shadcn/ui + Next.js). Slack/Telegram are **optional notification channels** (M3), not the MVP surface |
...
- **The webapp front-loads UI + auth into M0–M2** — more to build before the MVP than the old Slack-only plan; accepted for one coherent surface. **Telegram leaves the critical path** (optional notification).
