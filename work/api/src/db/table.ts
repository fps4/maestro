/**
 * The table, stated once (maestro ADR-0018, ADR-0019 §1).
 *
 * One table per component, keyed `pk`/`sk`, two general indexes and one sparse index for the
 * outbox, TTL on `expires_at` — the same shape as every other component's; only the key layout
 * (`keys.ts`) is this service's. This file is the schema in the sense that matters: what the key is,
 ...
 * `terraform/main.tf` says the two must match, and `Store.connect` checks the index names at boot
...
/** Epoch seconds. The idempotency caches — deliveries and requests — expire; nothing else does. */
