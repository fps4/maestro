...
- ~~**Diff-of-artefact mechanics.**~~ **Resolved 2026-05-28: committed refs only.** The agent commits its re-draft via the github adapter ([ADR-0017](decisions/0017-github-app-and-webhook-ingestion.md)); the SpecIndex re-indexes on push; the workspace renders `current_ref vs last_seen_ref` from the [read API](contracts/workspace-read-api.md). Uncommitted work-in-progress doesn't exist as far as the workspace is concerned — the agent either pushes a commit and emits `agent_response.posted` ([ADR-0022](decisions/0022-agent-response-event.md)), or the cycle stays open. No in-flight render path.
- ~~**Catch-up state per participant.**~~ **Resolved by [ADR-0023](decisions/0023-catch-up-state-workspace-local.md)** (2026-05-28): a **workspace-local** `participant_view_marker` table (per-fetch `last_seen_seq`), **not** an event-log entry. The catch-up marker is a per-participant rendering convenience, not authoritative state; the ADR-0015 invariant holds.
- ~~**Inbox prioritisation across products.**~~ **Resolved 2026-05-28: unread-then-chronological.** Default order: (1) tasks with `unread > 0` (per [ADR-0023](decisions/0023-catch-up-state-workspace-local.md)) ahead of those without, (2) within each bucket, **most-recent gate or response first**, (3) within ties, products the architect dispatched most recently. No risk tiering until a risk model exists; no per-product pinning until the dogfood spans many products. Revisit when US-0030 S6 (M3) lands.
- ~~**Anchoring on free-form text.**~~ **Resolved 2026-05-28: markdown heading slugs in M1; block-id deferred.** Spec criteria (`AC-N`) are stable and addressable; technical-design sections use **markdown heading slugs** (`{ heading: "<slug>" }`) as the comment anchor — simple, dogfood-acceptable, fragile if an editor renames a heading. A renderer-assigned block-id approach is a deferred refinement when designs grow into more elaborate forms. Recorded as a known limitation in [`workspace-write-api.md`](contracts/workspace-write-api.md) §known-limitations.
